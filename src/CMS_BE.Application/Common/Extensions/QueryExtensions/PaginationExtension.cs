...
using CMS_BE.Application.Common.Extensions.Expressions;
using CMS_BE.Application.Common.Extensions.Reflections;
using Microsoft.EntityFrameworkCore;

namespace CMS_BE.Application.Common.Extensions.QueryExtensions
{
    public static class PaginationExtension
    {
        ...
        {
            int totalCount = await query.CountAsync(); // Truy vấn một lần
            int totalPage = totalCount > size ? (int)Math.Ceiling(totalCount / (double)size) : 1;
        ...
        public static PaginationResponse<T> ToPagedList<T>(
            ...
            new(
                query.Skip((current - 1) * size).Take(size),
                (query.Count() > size) ? (int)Math.Ceiling((query.Count() / (double)size)) : 1,
                current,
                size
            );
        ...
        {
            BinaryExpression? innerExpression = BuildEqualOperationClause(
                index,
                CompararisonValues
            );
        ...
        )
        {
            ...
            if (memberType == typeof(DateTimeOffset) || memberType == typeof(DateTimeOffset?))
            {
                ...
                {
                    string stringValue => DateTimeOffset.TryParse(
                        stringValue,
                ...
                if (finalValue == null && value != null)
                {
                    throw new InvalidOperationException(
                        $"Cannot convert value '{value}' to {memberType}."
                    );
                }

                return new(member, Expression.Constant(finalValue, memberType));
            }
            ...
            if (memberType.IsNullable())
            {
                ...
                return new(member, Expression.Constant(convertedValue, memberType));
            }
            ...
            if (value != null && memberType != value.GetType())
            {
                return new(
                    member,
                    Expression.Constant(Convert.ChangeType(value, memberType), memberType)
                );
            }
