...
sql() {
  echo "Running: scp -r -P $REMOTE_SERVER_PORT $REMOTE_SERVER_USER@$REMOTE_SERVER_IP:~/clinic-management-system-be/.pgdump ."
  scp -r -P $REMOTE_SERVER_PORT $REMOTE_SERVER_USER@$REMOTE_SERVER_IP:~/clinic-management-system-be/.pgdump .
}
sql_to_server() {
  echo "Running: scp -r -P $REMOTE_SERVER_PORT .pgdump \
    $REMOTE_SERVER_USER@$REMOTE_SERVER_IP:~/clinic-management-system-be/"
